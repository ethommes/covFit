...
# _v6: some plotting glitches fixed

find_exponential_portion_v7 <- function(incidence_list,
                                         population,
                                         ...
                                         post_turnover_up_to_present_TF,
                                         sigma_SEIR,
                                         gamma_SEIR,
                                         plot_only_linear_TF,
                                         plot_TF,
  ...
  if (is.na(onset_date_override)) {
    ...
    frame_for_lm_backup <- frame_for_lm # backup, for when we (possibly) modify it below for post-turnover portion
  }
  ...
  if (cases_total >= 0) {
    ...
    frame_for_lm <- frame_for_lm_backup
    latest_index <- nrow(frame_for_lm)
    first_current_index <- max(1,latest_index - window_for_current_R)
    ...
    if (nrow(current_fit_summary$coefficients) > 1) {
    ...
    if (plot_TF & plot_current_R_TF) {
      ...
      # TO DO: Also write output to screen if desired
      if (plot_to_screen_TF) {
        # browser()
        # p1 <- grid.draw(twopanel_plot)
        # print(p1)
      }
      
      # ggsave(path=pathname, filename=paste0(filename,".jpg"), twopanel_plot)
      ggsave(path=pathname, filename=filename, finished_plot)
