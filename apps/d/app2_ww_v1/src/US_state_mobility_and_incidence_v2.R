...
# _v2:  Uses region_mobility_and_incidence_v6().  Uses R0, R_mid, R_min, R_max, current incidence
#       from rolling_R_and_incidence()

US_state_mobility_and_incidence_v2 <- function(input) {
  with(input,{
    ...
    df <- data.frame(
      ...
      "rho_0" = rep(NA,n_rows),
      "R0" = rep(NA,n_rows),
      "rho_mid" = rep(NA,n_rows),
      "rho_min" = rep(NA,n_rows),
      "rho_max" = rep(NA,n_rows),
      "R_mid" = rep(NA,n_rows),
      "R_min" = rep(NA,n_rows),
      "R_max" = rep(NA,n_rows),
      # "rho"=rep(NA,n_rows),
    ...
    # for (i in 5:5) {
      ...
      df$rho_0[i] <- state_output$rho_0
      df$R0[i] <- state_output$R0
      df$rho_mid[i] <- state_output$rho_mid
      df$rho_min[i] <- state_output$rho_min
      df$rho_max[i] <- state_output$rho_max
      df$R_mid[i] <- state_output$R_mid
      df$R_min[i] <- state_output$R_min
      df$R_max[i] <- state_output$R_max
