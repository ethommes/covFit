...
find_exponential_portion_v3f <- function(incidence_list,
  ...
  filename <- paste(title,"_exponential_growth.png",sep="")
  ...
  # TEST: make also a plot of incidence per 100k population, and in log base 10:
  intercept2 <- (fit_summary$coefficients["(Intercept)","Estimate"] - log(population) + log(1e5))/log(10)
  slope2 <- fit_summary$coefficients["dates","Estimate"]/log(10)
  
  the_plot_TEST <- ggplot(data=incidence_aggr,aes(x=dates,y=log(cases/population*1e5)/log(10))) + 
    geom_point() +
    geom_point(data=exponential_portion,aes(x=dates,y=(logy - log(population) + log(1e5))/log(10)), color="red") +
    # geom_point(data=post_exponential_portion,aes(x=dates,y=logy), color="blue") +
    geom_abline(intercept=intercept2,
                slope=slope2,color="red") +
    # geom_abline(intercept=intercept_post_exp,
    #             slope=slope_post_exp,
    #             color = "blue") +
    annotate_textp(x=0.02,y=0.98,label=paste(
      ...
      "                 = ",round(R_percent_reduction,2),"% reduction\n",
      sep=""),size=9) +
    labs(title = paste("Initial exponential growth, ",title,sep=""), x="date", y = paste("log10(cases per ",N_days_to_aggregate,"days per population)"))
  
  if (!is.na(slope_post_exp)) {
    # intercept2 <- (fit_summary$coefficients["(Intercept)","Estimate"] - log(population) + log(1e5))/log(10)
    # slope2 <- fit_summary$coefficients["dates","Estimate"]/log(10)
    intercept_post_exp_scaled <- (intercept_post_exp - log(population) +log(1e5))/log(10)
    slope_post_exp_scaled <- slope_post_exp/log(10)
    the_plot_TEST <- the_plot_TEST +
      geom_point(data=post_exponential_portion,aes(x=dates,y=(logy - log(population) + log(1e5))/log(10)), color="blue") +
      geom_abline(intercept=intercept_post_exp_scaled,
                  slope=slope_post_exp_scaled,
                  color = "blue")
  }
  
  
  filename <- paste(title,"_exponential_growth_per_pop.png",sep="")
  ggsave(path = pathname, filename = filename,width=6,height=4)
