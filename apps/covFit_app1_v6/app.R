...
server <- function(input, output, session) {
    ...
    countries_vector <- reactive({
      if (input$world_or_US == "world") {
        countries_vector <- unique(covid19$Country.Region)
    ...
    regions_vector <- reactive({
      if (input$world_or_US == "world") {
        ...
        regions_vector <- c("ALL", unique(df_temp$Province.State))
        ...
        regions_vector <- unique(covid19_US$Province_State)
      }
    })
    observe({
    ...
    subregions_vector <- reactive({
      if (input$world_or_US == "world") {
        subregions_vector <- c("ALL") 
      } else {
        df_temp <- covid19_US %>% subset(Province_State == input$regionSelect)
        subregions_vector <- c("ALL", df_temp$Admin2)
      }
    })
    observe({
