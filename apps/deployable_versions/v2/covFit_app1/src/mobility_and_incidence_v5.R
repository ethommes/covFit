...
mobility_and_incidence_v5 <- function(incidence, 
                                   google_mobility,
                                   pop,
  ...
  initial_exp_growth_result <- find_exponential_portion_v3e(incidence,
                                                            pop,
  ...
  # Take third non-NA value of R as R_initial [needs refining]
  ...
  # plot2 <- ggplot(data=incidence,aes(x=dates,y=log(cases))) +
  #   geom_line() +
  #   geom_point() +
  #   geom_line(data=df,aes(x=date,y=average_roll/100*7)) +
  #   xlim(xlim)
  #   # geom_line(data=df,aes(x=date,y=average),color="green") 
  # #   scale_y_log10()
  # #   # scale_x_datetime(date_labels = "%a %H:%M:%S", date_breaks = "1 day")
  # print(plot2)
  # plot3 <- ggplot(data=incidence,aes(x=dates,y=slope)) + 
  #   geom_line() + 
  #   # geom_line(data=df,aes(x=date, y = average_mobility_roll/100),color="green")
  #   geom_line(data=df,aes(x=date, y = driving_roll/100),color="black") +
  #   geom_line(data=df,aes(x=date, y=transit_roll/100), color = "blue") +
  #   geom_line(data=df, aes(x = date, y = walking_roll/100), color = "green") +
  #   labs(title="rho + mobility") +
  #   xlim(xlim)
  # print(plot3)
  # plot4 <- ggplot(data=incidence,aes(x=dates,y=doubling_time)) + geom_line() + labs(title="doubling time") + ylim(c(-100,100)) + xlim(xlim)
  # print(plot4)
  
  # hack to avoid having to reshape data frames:
  colors <- c("retail + recr (G)" = "blue",
              "grocery + pharm (G)" = "red",
              "parks (G)" = "green",
              "transit (G)" = "darkblue",
              "workplaces (G)" = "orange",
              "residential (G)" = "darkblue",
              "R pct change" = "purple")
              # "driving (A)" = "black",
              # "walking (A)" = "darkgreen",
              # "transit (A)" = "cyan")
  plot5 <- ggplot(data = google_mobi$df,aes(date, recreation_roll, color = "retail + recr (G)")) + geom_line() +
    ...
    geom_line(data=incidence,aes(x=dates,y=R_over_R_initial, color = "R pct change"),size=1) +
    # geom_line(data=df,aes(x=date,y=driving_roll, color = "driving (A)"),linetype = 2) +
    # geom_line(data=df,aes(x=date,y=walking_roll, color = "walking (A)"),linetype = 2) +
    # # geom_line(data=df,aes(x=date,y=transit_roll, color = "transit (A)"),size=1,linetype=2)
    # geom_line(data=df,aes(x=date,y=transit_roll, color = "transit (A)"),linetype = 2) +
    labs(x= "date", y="percent baseline", color="Legend") +
    scale_color_manual(values=colors) 
  print(plot5)
  return(list("incidence"=incidence,"google_mobility"=google_mobi))
