...
server <- function(input, output, session) {
  ...
  # country_choices_list <- reactive({
  #   world_or_US <- input$world_or_US
  #   if (world_or_US == "world") {
  #     country_choices_list <- unique(covid19$Country.Region)
  #   } else {
  #     country_choices_list <- c("US")
  #   }
  ...
  observeEvent(input$world_or_US, {
    if (input$world_or_US == "world") {
      country_choices_list <- unique(covid19$Country.Region)
    } else {
      country_choices_list <- c("US")
    }
    updateSelectInput(session, inputId = "countrySelect", choices = country_choices_list)
  })
  
  # region_choices_list <- reactive({
  #   world_or_US <- input$world_or_US
  #   countrySelect <- input$countrySelect
  #   if (world_or_US == "world") {
  #     df_temp <- covid19 %>% subset(Country.Region == input$countrySelect)
  #     region_choices_list <- unique(df_temp$Province.State)
  #   } else if (world_or_US == "US") {
  #     region_choices_list <- unique(covid19_US$Province_State)
  #   }
  # })
  ...
  observeEvent(input$countrySelect, {
    if (input$world_or_US == "world") {
      df_temp <- covid19 %>% subset(Country.Region == input$countrySelect)
      region_choices_list <- c("ALL", unique(df_temp$Province.State))
    } else {
      region_choices_list <- unique(covid19_US$Province_State)
    }
    updateSelectInput(session, inputId = "regionSelect", choices=region_choices_list,selected = region_choices_list[1])
  })
  
  observeEvent(input$makePlot,{
    output$Plot <- renderPlot({
      if (isolate(input$world_or_US == "world")) {
        if (isolate(input$regionSelect) == "ALL") {
          region <- ""
        } else {
          region <- isolate(input$regionSelect)
        }
            
          
        temp <- non_US_mobility_and_incidence_v2(country = isolate(input$countrySelect),
                                                 region = region,
                                                 subregion = "",
        ...
        inp$Country.Region <- isolate(input$countrySelect)
        inp$Province.State <- isolate(input$regionSelect)
        inp$Admin2 <- ""
