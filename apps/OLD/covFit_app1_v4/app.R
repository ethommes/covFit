...
server <- function(input, output, session) {
  ...
  # input_intermediate is intermediate input:
  input_intermediate <- reactive({
    inp <- base_input_list
    ...
    input_intermediate <- list("inp" = inp, "gui_input" = input)
  })
  
  output$Plot <- renderPlot({
    input2 <- input_intermediate()
    if (input2$gui_input$world_or_US == "world") {
      temp <- non_US_mobility_and_incidence_v2(country = input2$gui_input$countrySelect,
                                               region = input2$gui_input$regionSelect,
                                               subregion = input2$gui_input$subregionSelect,
                                               correction_factor = NA, # hard-code for now
                                               covid_data = covid19,
                                               covid_death_data = covid19_deaths,
                                               plot_only_linear_TF = F,
                                               base_input = input2$inp)
      plot_to_render <- temp$plot
    } else {
      inp <- input2$inp
      inp$covid_data <- covid19_US
      inp$covid_death_data <- covid19_deaths_US
      inp$Country.Region <- input2$gui_input$countrySelect
      inp$Province.State <- input2$gui_input$regionSelect
      inp$Admin2 <- input2$gui_input$subregionSelect
      inp$mobility_subregion <- input2$gui_input$subregionSelect
