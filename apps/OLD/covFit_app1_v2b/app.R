...
server <- function(input, output, session) {
    ...
    observeEvent(input$plot_button, {
      output$Plot <- renderPlot({
        ...
        inp$predict_from_date <- isolate(input$predict_from_date)
        inp$predict_date <- isolate(input$predict_date)
        inp$R_window_size <- isolate(input$R_window_size)
        if (isolate(input$manual_y_scale)) {
          inp$manual_max_incidence <- isolate(input$manual_max_incidence)
        } else {
          inp$manual_max_incidence <- NA
        }
        if (isolate(input$apply_CFR_correction)) {
          inp$CFR <- isolate(input$CFR)
        } else {
          inp$CFR <- NA
        }
        # if (input$CFR > 0) {
        ...
        if (isolate(input$world_or_US == "world")) {
          temp <- non_US_mobility_and_incidence_v2(country = isolate(input$countrySelect),
                                                   ...
                                                   subregion = isolate(input$subregionSelect),
          ...
          inp$Admin2 <- isolate(input$subregionSelect)
